...
v1.4.14 已正式发布并完成公网生产切换；API/Web 健康和 v1.4.14 静态资源缓存标记均已核验。

## 交付范围

- 使用指南默认展示“边做边学”，原文字说明保留。
- 十个课程：认识界面、设备查询与预约、新增设备、取消本人预约、会议室预约、维修保养、采购验收、创建成员、新增与编辑实验室、系统升级演练。
- 按当前角色展示 7 / 9 / 10 个教程；模拟角色只改变教程目录，不赋予任何真实权限。
...
- 本次 Safari 修复已包含在 v1.4.11 Release；未执行生产接口操作或切换生产。

### 新增设备教程（v1.4.12，随 v1.4.14 保留）

- 新增四步课程：设备台账 → 新增设备 → 填写正式表单 → 核对台账新行。普通用户/管理员/开发者分别为 7/9/10 个教程。
- 字段和校验与现有业务对齐：编号非空、最多 80 字符并标准化为大写；性能指标最多 500 字符；实验室选择提交名称；四种初始状态可演练。演示 API 校验重复编号、必填/长度和已启用实验室，失败时不修改虚拟记录或审计。
- 已验证自填字段与非默认状态回放、返回上一步后重新提交不重复、重启清空演示数据。
- `pnpm check`：144/144 通过。Chromium 实际操作完成全部十课；WebKit 新增设备手动练习、自动演示及 390×844 手机录入流程通过。测试期间业务网络请求为零。
- 本版已纳入 v1.4.14 Release；公网生产是否切换由部署方按备份、升级代理和 Ubuntu 专项验证结果决定。
