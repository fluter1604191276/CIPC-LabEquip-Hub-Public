...
<a id="upgrade-v1-4-12"></a>

## v1.3.0 至 v1.4.11 快速升级到 v1.4.12

如果学校已经按旧版部署，推荐从公开仓库标签获取升级脚本。它会自动备份数据库、获取 v1.4.12、运行检查、更新 API 单元、安装升级中心并重建 Web；不会删除生产数据库。升级前请确认服务器已能访问 GitHub，并预留维护窗口：

```bash
cd /opt/cipc-labequip/current
git fetch --tags origin
git show v1.4.12:scripts/upgrade-lan-from-v1.3.sh > /tmp/upgrade-lan-from-v1.3.sh
sudo bash /tmp/upgrade-lan-from-v1.3.sh v1.4.12
```
...
生产 v1.4.11 的旧升级单元应重新安装带 `--flock-held` 的 v1.4.12 单元，避免外层 flock 与代理内部锁重复：

```bash
cd /opt/cipc-labequip/current
sudo install -m 0644 deploy/lan/cipc-labequip-upgrade.service /etc/systemd/system/cipc-labequip-upgrade.service
sudo systemctl daemon-reload
sudo systemctl restart cipc-labequip-upgrade.path
```

## v1.4.11 交互式使用指南与 Safari 修复

v1.4.11 的“边做边学”直接复用正式版本的页面、样式、表单和交互，只替换为严格隔离的内存数据适配器。普通用户、管理员和开发者分别看到对应教程；系统升级课程只模拟流程，不执行真实部署。

Safari/WebKit 下，教程的两层演示 iframe 同时启用 `allow-scripts` 以保留宿主事件处理；子文档仍由 `script-src 'none'`、`connect-src 'none'`、`form-action 'none'` 和脚本剥离保护，所有业务请求强制进入内存适配器。公网生产仍保持 v1.4.10，本版本可先在测试环境验证。
...
v1.4.9 将 Web 的 CSS/JavaScript 资源查询版本同步到发布版本，避免浏览器继续复用旧的 `app.js`，从而确保 v1.4.8 的手动升级显示修复立即生效。
