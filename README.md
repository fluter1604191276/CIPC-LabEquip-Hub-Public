...
> `v1.4.10` 强化系统升级页的“检查更新”入口，增加清晰的同步图标、主色按钮、说明文字和检查中反馈；同时保留此前的升级稳定性修复。新部署可固定使用 `v1.4.10` 标签；已部署环境可从升级中心验证自动升级。版本差异见 [版本矩阵](docs/VERSION_MATRIX.md)。
...
已经部署 v1.3.0 及之后版本的学校，可直接执行 [快速升级命令](docs/RELEASE_AND_MIGRATION.md#upgrade-v1-4-10)。安装命令和回滚说明见 [发布、迁移与部署流程](docs/RELEASE_AND_MIGRATION.md) 以及 [局域网部署说明](deploy/lan/README.md)。数据库回滚仍需管理员按备份恢复流程显式执行。
...
- [v1.4.9 发布说明](docs/RELEASE_NOTES_v1.4.9.md)
