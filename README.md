...
> `v1.4.11` 已发布交互式使用指南和 Safari 修复；当前本地正在准备 v1.4.12 候选补充“新增设备”教程，尚未发布。新部署可固定使用 `v1.4.11` 标签；已部署环境可从升级中心验证自动升级。版本差异见 [版本矩阵](docs/VERSION_MATRIX.md)。
...
已经部署 v1.3.0 及之后版本的学校，可直接执行 [快速升级命令](docs/RELEASE_AND_MIGRATION.md#upgrade-v1-4-11)。安装命令和回滚说明见 [发布、迁移与部署流程](docs/RELEASE_AND_MIGRATION.md) 以及 [局域网部署说明](deploy/lan/README.md)。数据库回滚仍需管理员按备份恢复流程显式执行。
